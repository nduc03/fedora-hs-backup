...
make_config_dir() {
    echo ">>> Creating config directories, copying files and adjusting permissions if necessary..."

    for DIR_NAME in "$@"; do
        local TARGET="$SERVICE_DATA_DIR/$DIR_NAME"
        local SOURCE="$SCRIPT_DIR/$DIR_NAME"
        echo "--- Đang xử lý config dir: $TARGET ---"
        ...
        # 2. Copy toàn bộ file config
        if [ -d "$SOURCE" ]; then
            echo ">> Copying config files from $SOURCE to $TARGET..."

            # Reset quyền về user hiện tại để tránh lỗi "Permission denied" ở lần chạy thứ 2
            # do thư mục đang bị giữ bởi UID của Podman (subuid)
            if [ -d "$TARGET" ] && [ "$ROOTLESS" = "true" ]; then
                sudo chown -R $(id -u):$(id -g) "$TARGET"
            fi

            cp -r "$SOURCE/." "$TARGET/"

            # Đảm bảo quyền đọc/ghi cơ bản cho thư mục và file config
            find "$TARGET" -type d -exec chmod 755 {} \;
            find "$TARGET" -type f -exec chmod 644 {} \;
        else
            echo ">> [WARNING] Source config directory $SOURCE does not exist. Skipping copy."
        fi
...
EOF
  ...
  # Không dùng template -> cài thẳng từ file gốc, không qua TMP_DIR
  QUADLET_FILE_LOCATION="$SCRIPT_DIR/$QUADLET_FILENAME"
fi

# Ngừng script sớm nếu đang bật chế độ debug
if [[ "$DEBUG_MODE" == "true" && "$USE_TEMPLATE" == "true" ]]; then
  echo ">>> [DEBUG MODE] All templates generated successfully in: $DEBUG_OUT_DIR"
  exit 0
elif [[ "$DEBUG_MODE" == "true" ]]; then
  echo ">>> [DEBUG MODE] USE_TEMPLATE=false, không có gì để debug template. Bỏ qua."
fi
...
make_config_dir "${CONFIG_DIR_NAMES[@]}"
