...
#* if you use traefik but need to fine-tune the labels, don't set to true, specify labels in .template instead
#* remember to set `==%traefik_labels%==` in a separate line under [Container] in your .template file
...
pre_install() {
  ...
  podman unshare chown -R 10001:10001 data logs
...
#* CTV["%app_port%"]="8080"
#* tạo file, đinh nghĩa và sử dụng biến ở trong ctv.env nếu cần
...
Label=traefik.http.routers.$SERVICE_NAME.rule=Host(\`$SERVICE_NAME.hs.lan\`)
...
if [[ "$USE_TEMPLATE" = "true" ]]; then
 ...
 # Inject TRAEFIK_LABELS if USE_TRAEFIK_LABELS is true, otherwise clear the placeholder
