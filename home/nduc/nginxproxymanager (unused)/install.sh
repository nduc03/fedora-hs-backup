...
ROOTLESS=false #* set to true if this container can run rootless
...
echo ">>> Setting SELinux context for pre-setup.sh"
sudo semanage fcontext -a -t bin_t "$SCRIPT_DIR/pre-setup.sh"
sudo restorecon "$SCRIPT_DIR/pre-setup.sh"
#* ...
