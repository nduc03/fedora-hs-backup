...
pre_install() {
  ...
  mkdir -p "$SERVICE_DATA_DIR/mycert"
  mkdir -p "$SERVICE_DATA_DIR/config"
  mkdir -p "$SERVICE_DATA_DIR/secret"

  cp "$SCRIPT_DIR/config/*" "$SERVICE_DATA_DIR/config/"

  echo "CF_DNS_API_TOKEN=" > "$SERVICE_DATA_DIR/secret/cloudflare.env"

  EDIT="n"
  read -p "cloudflare.env has been created, do you want to edit it now? (y/n) " EDIT
  if [[ "$EDIT" == "y" ]]; then
    nano "$SERVICE_DATA_DIR/secret/cloudflare.env"
  fi
